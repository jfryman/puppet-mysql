class mysql::config {
  ...
  exec { 'setup-mysql-backup-user':
    command => "mysql -uroot -p${mysql::params::ml_root_password} -e 'GRANT SHOW DATABASES, SELECT, LOCK TABLES, RELOAD ON *.* to ${mysql::params::ml_backup_user} IDENTIFIED BY ${mysql::params::ml_backup_password}'",
